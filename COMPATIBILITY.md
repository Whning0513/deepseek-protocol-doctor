...
## FlowDown Fireworks DeepSeek V4 online E2E capture

- FlowDown [commit `c482f8432e411c09a71c803bc24295970c86d0ce`](https://github.com/Lakr233/FlowDown/commit/c482f8432e411c09a71c803bc24295970c86d0ce) was checked on 2026-08-15 UTC. The commit was created on 2026-08-12. Its `online_e2e/cases` directory publishes the matching [request](https://github.com/Lakr233/FlowDown/blob/c482f8432e411c09a71c803bc24295970c86d0ce/Resources/DevKit/fixtures/online_e2e/cases/tool_add_numbers_call.request.json), [meta](https://github.com/Lakr233/FlowDown/blob/c482f8432e411c09a71c803bc24295970c86d0ce/Resources/DevKit/fixtures/online_e2e/cases/tool_add_numbers_call.meta.json), and [response](https://github.com/Lakr233/FlowDown/blob/c482f8432e411c09a71c803bc24295970c86d0ce/Resources/DevKit/fixtures/online_e2e/cases/tool_add_numbers_call.response.sse) files.
- The source meta records `status: 200`, `content_type: text/event-stream`, `stream: true`, the upstream endpoint `https://api.fireworks.ai/inference/v1/chat/completions`, and model `accounts/fireworks/models/deepseek-v4-pro`. This is repository-supplied capture metadata; this project did not rerun the upstream request.
- The source request has one synthetic user message, one strict `add_numbers` function with required integer properties `a` and `b`, `stream: true`, `temperature: 0`, and `max_completion_tokens: 256`. The repository fixture [`flowdown_fireworks_deepseek_v4_tool_call.request.json`](fixtures/flowdown_fireworks_deepseek_v4_tool_call.request.json) replaces the user content with `<redacted-user-content>`.
- The source response file is 10,650 bytes with blob SHA `08b9559e2e9fc667822adf77faa535a863c6de83`. It contains 38 JSON data chunks and one `data: [DONE]` line. The stream has 33 `reasoning_content` deltas, a content delta containing `\n\n`, one index-0 `add_numbers` tool-call delta with `{"a": 17, "b": 25}`, a `finish_reason: "tool_calls"` chunk, and a later `choices: []` usage chunk. The repository fixture [`flowdown_fireworks_deepseek_v4_tool_call.sse`](fixtures/flowdown_fireworks_deepseek_v4_tool_call.sse) normalizes the completion ID and tool-call ID consistently.
- Running the current doctor against the source request produces `MAX_TOKENS_MISSING` because it uses `max_completion_tokens`, plus `STRICT_ADDITIONAL_PROPERTIES` because the strict schema omits `additionalProperties: false`. Those findings are retained as observed compatibility evidence; the request is not rewritten to make it pass. Running the stream inspector against the source response returns `ok=true` with the tolerated `SSE_EMPTY_CHOICES` information finding.
- The source layout and meta support treating this file as a public provider capture for this exact Fireworks/FlowDown case. It does not establish a universal Fireworks, DeepSeek V4, or OpenAI-compatible contract. No new finding code is inferred from the capture.
